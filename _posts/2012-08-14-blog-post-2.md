...
There is a lack of generliazed datasets for tasks like opinion-mining and aspect based sentiment analysis. Most of the adequately sized datasets for these tasks are heavily dependent on the domain of the data. Ex: SemEval-2014 Restaurants & Laptops reviews data, FiQA-2018 Financial Microblogs Data etc Any learning based models for these tasks trained on these kind of datasets won't deliver on data from sources other than restaurant reviews, laptop reviews etc 
...
Using the POS Tagger, Dependency Parseer and Co-reference resolution tool from the Stanford CORENLP-toolkit, these types of results can be obtained :
